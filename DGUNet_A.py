...
class DGUNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=32, scale_unetfeats=16, scale_orsnetfeats=16, num_cab=8, kernel_size=3,
        ...
        # self.Phi = nn.Parameter(init.xavier_normal_(torch.Tensor(np.ceil(cs_ratio*0.01*256.).astype(np.int), 256)))
        act = nn.PReLU()
    ...
    def forward(self, img, A):
        ...
        for _ in range(self.nums_stages):
            stage_img, x_samfeats, feat1, f_decoder = self.stage_model(stage_img, img, x_samfeats, feat1, f_decoder,
                                                                       PhiTPhi, PhiTb)
            output_.append(stage_img)
        ##-------------------------------------------
        ##-------------- Stage k---------------------
...
#model_restoration = DGUNet(in_c=1, out_c=1, cs_ratio=50)
#model_restoration.cpu()
#x = torch.rand((10, 1, 16, 16)).cpu()
#A = torch.ones((128, 256))
#y = model_restoration(x, A)
1